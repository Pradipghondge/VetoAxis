...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userRole !== 'super_admin') {
      ...
      if (!user?.organizationId) return NextResponse.json({ statusCounts: [], totalLeads: 0, recentActivity: [], chartData: [] });
    ...
    const chartDataRaw = await Lead.aggregate([
      { $match: { ...organizationFilter, createdAt: { $gte: twentyFourHoursAgo } } },
      {
        $group: {
          _id: {
            hour: { $hour: "$createdAt" },
